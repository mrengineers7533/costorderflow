ALTER TABLE public.boqs ADD COLUMN IF NOT EXISTS show_motor boolean NOT NULL DEFAULT true;

CREATE OR REPLACE FUNCTION public.verify_boq_items_with_token(_token uuid, _verifier_email text, _items jsonb, _show_motor boolean DEFAULT NULL)
 RETURNS boqs
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.boqs
     SET line_items = _new_items,
         ...
         show_motor = COALESCE(_show_motor, show_motor),
         updated_at = now()
   WHERE id = _row.id
   RETURNING * INTO _row;
